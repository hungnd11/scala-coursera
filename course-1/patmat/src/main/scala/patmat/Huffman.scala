...
object Huffman {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    val ordered = freqs.sortWith(_._2 < _._2)
    
    def makeLeaftList(ordered: List[(Char, Int)], acc: List[Leaf]): List[Leaf] = {
      if(ordered.isEmpty)
        acc
      else 
        Leaf(ordered.head._1, ordered.head._2) :: makeLeaftList(ordered.tail, acc)
    }

    makeLeaftList(ordered, List())
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    val charsRepByTree = chars(tree)
    def makeTable(cs: List[Char]): CodeTable =
      if(cs.isEmpty) 
        List()
      else 
        (cs.head, encode(tree)(List(cs.head))) :: makeTable(cs.tail)

    makeTable(charsRepByTree)
  }
