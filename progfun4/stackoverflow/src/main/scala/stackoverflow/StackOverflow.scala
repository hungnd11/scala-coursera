...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val answers = postings.filter(a => a.postingType == 2 && a.parentId.isDefined)
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    // grouped.map(v => (v._2.head._1, answerHighScore(v._2.map(_._2))))
  }
  ...
  def getMedian(xs: List[Int]): Int = {
    val ys = xs.sortWith(_ < _)
    if (xs.length % 2 == 0) {
      val startIndex = xs.length / 2 - 1
      val droppedSeq = ys.drop(startIndex)
      (droppedSeq.head + droppedSeq.tail.head) / 2
    } else {
      ys.drop(xs.length / 2).head
    }
  }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs => 
      val mostCommonLangIndex = vs.groupBy(_._1).maxBy(_._2.size)._1

      val langLabel: String   = langs(mostCommonLangIndex / langSpread)
      val clusterSize: Int    = vs.size      
      val langPercent: Double = 100 * vs.count(_._1 == mostCommonLangIndex) / clusterSize.toDouble
      val medianScore: Int    = getMedian(vs.map(_._2).toList)
